...
const response = await sendRequest("../api/get_student_classroom_selected_post", "POST", {
...
announcementButton.addEventListener('click', async function () {
    ...
    showLoadingModal(); 
    const response = await sendRequest("../api/reply_student_classroom_post", "POST", { 
        'post_id' : post_id,
...
async function get_classroom_materials(){
    ...
    const response = await sendRequest("../api/get_student_classroom_post_replies", "POST", {
        'post_id' : classroom_id 
    }); 
